from networksecurity.entity.artifact_entity import DataIngestionArtifact, DataValidationArtifact #the input of the data validation will be coming from this pipeline
from networksecurity.entity.config_entity import DataValidationConfig
from networksecurity.exception.exception import NetworkSecurityException
from networksecurity.constant.training_pipeline import SCHEMA_FILE_PATH
from networksecurity.utils.main_utils.utils import read_yaml_file, write_yaml_file  ## reading the yaml file from the utils folder where all the generic code related to project will be written.
...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        """
        Validate if the dataframe contains numerical columns.
        """
        try:
            ...
            if len(numerical_columns) > 0:
                return True
            logging.error("No numerical columns found in the dataset.")
            return False
        except Exception as e:
            raise NetworkSecurityException(e, sys)
